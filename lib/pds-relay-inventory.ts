...
export function parseRelayListHostsPage(
  ...
  for (const raw of page.hosts) {
    ...
    instances.set(serviceHost, {
      ...
      relayAccountCount: integerOrNull(host.accountCount) ?? 0,
  ...
  return {
    ...
    cursor: typeof page.cursor === "string" && page.cursor.trim()
      ? page.cursor
      : null,
  };
}
...
export async function fetchRelayPdsInventory(
  ...
  while (pages < maxPages) {
    ...
    if (parsed.instances.length === 0 || !parsed.cursor) {
...
export function summarizeRelayPdsInventory(
  ...
  for (const instance of instances) {
    ...
    totalAccounts += instance.relayAccountCount;
    if (instance.isBlueskyHost) {
      blueskyInstances++;
      blueskyAccounts += instance.relayAccountCount;
    }
  }
  return {
...
export async function persistRelayPdsInventoryForClient(
  ...
  const observedAt = Math.max(0, Math.floor(options.observedAt ?? Date.now()));
